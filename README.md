...
absolute request URL. Patterns must start with `^`, end with `$`, and avoid
nested or ambiguous repetition. All matching rules are merged in order; later
explicit `methods`, header entries, and `timeout` values win. Header `add`
operations append across matching rules. Authentication belongs only to the last
matching rule, so a broader rule cannot accidentally inherit a narrower rule's
secret. A request that matches no rule is denied. An empty `methods: []`
deliberately denies all methods until a later, more specific rule enables them.
...
Inbound `authorization`, `cookie`, and `proxy-authorization` headers are
stripped by default. A rule can explicitly set
`request.forwardIncomingCredentials: true` when caller credentials are required.
Responses are buffered as UTF-8 text and limited to 10 MiB by default; binary
payloads are not supported by the current cloud protocol.
